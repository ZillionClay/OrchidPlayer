 ...
 * @li an @ref AVFormatContext.pb "I/O m_Context". It is either opened by lavf or
 ...
 * Further options may be provided by the @ref AVFormatContext.pb "I/O m_Context",
 ...
 * options on a preallocated m_Context. Instead, the options should be passed to
 ...
 * avformat_alloc_context() to create a muxing m_Context. The caller then sets up
 * the muxer by filling the various fields in this m_Context:
 *
 * - The @ref AVFormatContext.oformat "oformat" field must be set to select the
 *   muxer that will be used.
 * - Unless the format is of the AVFMT_NOFILE type, the @ref AVFormatContext.pb
 *   "pb" field must be set to an opened IO m_Context, either returned from
 ...
 *   remuxing: there is no guarantee that the codec m_Context values remain valid
 ...
 *   support.
 *
 * When the muxing m_Context is fully set up, the caller must call
 * avformat_write_header() to initialize the muxer internals and write the file
 * header. Whether anything actually is written to the IO m_Context at this step
 ...
 * a single muxing m_Context, they should not be mixed). Do note that the timing
 ...
 * m_Context (if any) and finally free the muxing m_Context with
 ...
 *
 * @param s    associated IO m_Context
 * @param pkt packet
...
typedef struct AVOutputFormat {
    ...
    const AVClass *priv_class; ///< AVClass for the private m_Context
} AVOutputFormat;
/**
 * @}
 */
...
typedef struct AVInputFormat {
    ...
    /**
     * Stop playing - only meaningful if using a network-based format
     * (RTSP).
     */
    ...
    /**
     * Returns m_Device list with it properties.
...
typedef struct AVFormatContext {
     ...
     *           take care of closing / freeing the IO m_Context.
     *
...
#define AVFMT_FLAG_SHORTEST   0x100000 ///< Close muxing when the shortest stream stops.
...
#define AVFMT_AVOID_NEG_TS_MAKE_ZERO         2 ///< Shift timestamps so that they start at 0
    ...
    /**
     * Flush the I/O m_Context after each packet.
     * - encoding: Set by user
     * - decoding: unused
     */
    int flush_packets;
     ...
     * This is set by avformat when the underlaying IO m_Context read pointer
     ...
     * other times as well), it will call this callback to obtain an IO m_Context.
     *
     * @param s the format m_Context
     * @param pb on success, the newly opened IO m_Context should be returned here
     ...
     *
     * @param s the format m_Context
     * @param pb IO m_Context to be closed and freed
     * @return 0 on success, a negative AVERROR code on failure
     */
 ...
 * avformat_free_context() can be used to free the m_Context and everything
 ...
 * avformat_free_context() can be used to free the m_Context and
 * everything allocated by the framework within it.
 *
 * @param ctx           pointee is set to the created format m_Context,
 *                      or to NULL in case of failure
 * @param oformat       format to use for allocating the m_Context, if NULL
 *                      format_name and filename are used instead
 * @param format_name   the name of output format to use for allocating the
 *                      m_Context, if NULL filename is used instead
 * @param filename      the name of the filename to use for allocating the
 *                      m_Context, may be NULL
 *
 ...
 * @param logctx         the log m_Context
 ...
 *
 * @note If you want to use custom IO, preallocate the format m_Context and set its pb field.
 */
...
/**
 * Stop a network-based stream (e.g. RTSP stream).
 *
 ...
 * codec and time base.
 *
 * @param ic        the m_Context to analyze
 * @param index     index of the stream to dump information about
 * @param url       the URL to print, such as source or destination file
 * @param is_output Select whether the specified m_Context is an input(0) or output(1)
 */
 ...
 *           array is composed by only one m_Context, such m_Context can contain
 ...
 *
 * @param format the format m_Context which the stream is part of
 ...
 *
 * @param ctx the format m_Context which the stream is part of
 * @param stream the stream which the frame is part of
