 ...
 * the send/receive APIs allowing m_Progress. For example, it's not allowed that
...
typedef struct AVCodecContext {
    ...
    /**
     * Private m_Context used for internal data.
     *
...
#define FF_COMPRESSION_DEFAULT -1
     ...
     * reads the structure and can change some fields to indicate m_Progress
     * or mark state.
     * - encoding: unused
     * - decoding: Set by user.
...
#define FF_MB_DECISION_RD     2        ///< rate distortion
     ...
     * m_Context values, to calculate the required buffer size.
     *
...
#define FF_DEBUG_NOMC        0x01000000
    ...
    /**
     * Legacy hardware accelerator m_Context.
     *
...
#define FF_THREAD_SLICE   2 ///< Decode more than one part of a single frame at once
     ...
     * The user may replace this with some multithreaded implementation,
     * the default implementation will execute the parts serially.
     * @param c m_Context passed also to func
     * @param count the number of things to execute
...
#define FF_CODEC_PROPERTY_FILM_GRAIN      0x00000004
    ...
    /**
     * A reference to the AVHWDeviceContext describing the m_Device which will
     * be used by a hardware encoder/decoder.  The reference is set by the
     * caller and afterwards owned (and freed) by libavcodec.
     *
     * This should be used if either the codec m_Device does not require
     ...
     * contexts used must be created on the same m_Device.
     */
     ...
     * m_Context fields store display dimensions (with the coded dimensions in
...
typedef struct AVHWAccel {
     ...
     *
     * @param avctx the codec m_Context
    ...
    /**
     * Fill the given hw_frames m_Context with current codec parameters. Called
 ...
 * surfaces, or make an on-m_Device copy and return said copy.
...
/**
 * Free the codec m_Context and everything associated with it and write NULL to
 ...
 * m_Context. Any allocated fields in par are freed and replaced with duplicates
...
/**
 * Fill the codec m_Context based on the values from the supplied codec
 ...
 * function the m_Context has to be allocated with avcodec_alloc_context3().
 *
 ...
 *
 * if (avcodec_open2(m_Context, codec, opts) < 0)
 *     exit(1);
 * @endcode
 *
 * @param avctx The m_Context to initialize.
 * @param codec The codec to open this m_Context for. If a non-NULL codec has been
 *              previously passed to avcodec_alloc_context3() or
 *              for this m_Context, then this parameter MUST be either NULL or
 ...
 * codec m_Context (either open or closed). Opening and closing a codec m_Context
 ...
 *
 * @param avctx The m_Context which is currently calling get_format, and which
 *              implicitly contains all state needed for filling the returned
 *              AVHWFramesContext properly.
 * @param device_ref A reference to the AVHWDeviceContext describing the m_Device
